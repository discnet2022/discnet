...
def get_threshold_by_fq(code2idx, code2fq, split_point=5, head_t=0.26, tail_t=0.1):
    idx2code = {v:k for k, v in code2idx.items()}
    threshold_lis = []
    for i in range(len(idx2code)):
        if code2fq[idx2code[i]] >= split_point:
            threshold_lis.append(head_t)
        else:
            threshold_lis.append(tail_t)
    return np.array(threshold_lis)
...
def all_metrics(yhat, y, k=8, yhat_raw=None, calc_auc=True):
    ...
    if yhat_raw is not None and calc_auc:
        ...
        auprc = auprc_metrics(yhat_raw, y, ymic)
        metrics.update(roc_auc)
        metrics.update(auprc)

    return metrics

def macro_precision_(yhat, y):
    num = intersect_size(yhat, y, 0) / (yhat.sum(axis=0) + 1e-10)
    return num
def macro_recall_(yhat, y):
...
def auprc_metrics(yhat_raw, y, ymic):
    # ymic = np.ravel(y)
    ...
    for i in range(y.shape[1]):
        ...
        if y[:,i].sum() > 0:
            _precision[i], _recall[i], _ = precision_recall_curve(y[:,i], yhat_raw[:,i])
            if len(_precision[i]) > 1 and len(_recall[i]) > 1:
                auc_score = auc(_recall[i], _precision[i])
    ...
    auprc['auprc_macro'] = np.mean(aucs)

    #micro-AUC: just look at each individual prediction
    yhatmic = yhat_raw.ravel()
    _precision["micro"], _recall["micro"], _ = precision_recall_curve(ymic, yhatmic)
    auprc["auprc_micro"] = auc(_recall["micro"], _precision["micro"])
